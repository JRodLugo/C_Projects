...
Kennel *getKennelByCat(CatStore *s, Cat *cat){
    ...
    for(int i = 0; i < s->numKenels; i++){
        ...
        //printf("Current kennel is %s", s->kennels[i].location);
...
int getCatPosi(Kennel *home, Cat *cat){
    //printf("getCatPosi is running\n");
    //loops through homes cats
    for(int i = 0; i < home->occupancy; i++){
        //if cat[i] is the same at the inputed cat
        //printf("getCatPosi is looping\n");
        //printf("kennel name %s\n", home->location);
        //printf("current cat %s\n", home->cats[i]->name);
        //printf("cat wanted %s\n", cat->name);
        if(home->cats[i] == cat){
            //printf("getCatPosi is ran returned index for %s name\n", home->cats[i]->name);
            return i;//return index
        }//end of if
    }//end of loop
    //if nothing is found
    //printf("getCatPosi is ran exit -1\n");
    return -1;
}//end of func
    ...
    for(int i = 0; i < numQueries; i++){
        ...
        if(userQuerie == 1){
            ...
            for(int j = 0; j < s->numKenels; j++){
                ...
                //printf("Current kennel is %s\n", s->kennels[j].location);
                destinationKennel = &s->kennels[j];
                //printf("I am looping it\n");
                //printf("destination kennel name %s\n", destinationKennel->location);
                //printf("Source kennel name %s\n", sourceKennel->location);
                ...
                if(sourceKennel == destinationKennel && dummyNum == 1){
                    ...
                    //printf("destination occupancy after %d\n", destinationKennel->occupancy);
                    destinationKennel->cats = (Cat*)realloc(destinationKennel->cats, destinationKennel->occupancy * sizeof(Cat*));
                    destinationKennel->cats = q3cat;
